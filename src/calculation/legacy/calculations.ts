 ...
 */
import type { LegacyPriceCatalog } from './embedded-default-prices.js';
...
const ClassicEngine = {
  calculate: (
    ...
    if (type === ProductType.FRAME) {
      ...
      // Legacy dynamic profile lookup (unchanged formula access pattern).
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      pPrice = (CF.profiles as any)[profKey]?.[lookupColor] ?? (CF.profiles as any)[profKey]?.white;
      cPrice = frameProfile === '25' 
 ...
 */
const RollEngine = {
  calculate: (wM: number, hM: number, mesh: MeshType, prices: LegacyPriceCatalog) => {
