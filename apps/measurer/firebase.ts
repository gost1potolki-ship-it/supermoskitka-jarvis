...
// Включаем офлайн-персистентность Firestore:
// записи сохраняются локально и синхронизируются при появлении сети.
enableIndexedDbPersistence(db).catch((err: { code?: string }) => {
  if (err?.code === 'failed-precondition') {
    console.warn('Firestore offline cache не включен: открыто несколько вкладок.');
    return;
  }
  if (err?.code === 'unimplemented') {
    console.warn('Firestore offline cache не поддерживается в этом браузере.');
    return;
  }
