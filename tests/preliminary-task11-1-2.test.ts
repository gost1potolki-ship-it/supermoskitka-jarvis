...
describe('Task 11.1.2 trusted quote & profitability invariants', () => {
  describe('TRUST proof provenance', () => {
    it('TRUST-1 production barrel has no fromTrustedLegacyCalculation', () => {
      expect(TrustedPreliminaryQuoteProof).toBeDefined();
      expect(
        Object.prototype.hasOwnProperty.call(
          TrustedPreliminaryQuoteProof,
          'fromTrustedLegacyCalculation',
        ),
      ).toBe(false);
      expect(
        (TrustedPreliminaryQuoteProof as { fromTrustedLegacyCalculation?: unknown })
          .fromTrustedLegacyCalculation,
      ).toBeUndefined();
    ...
    it('TRUST-2 create proof without trusted normalized input fails closed', () => {
      const memory = frameMemory();
      const result = createTrustedPreliminaryQuoteProof({
        memory,
        outcome: {
          status: 'calculated',
          items: [],
        ...
        // @ts-expect-error intentional missing trustedInput
        trustedInput: undefined,
      });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.code).toBe('TRUSTED_INPUT_REQUIRED');
      }
    });
    ...
    it('TRUST-5 quote public total equals exact legacy outcome total', async () => {
      for (const total of [8500, 9000, 9050, 15200]) {
        ...
        const proof = createTrustedPreliminaryQuoteProof({
          memory,
          outcome: {
            status: 'calculated',
            items: [],
        ...
        if (proof.ok) {
          expect(proof.proof.publicTotalRub).toBe(total);
        }
      }
    });
  });
  ...
  describe('readiness with non-blocking profitability', () => {
    it('RED/PARTIAL profitability does not block READY/AUTO_ALLOWED', () => {
